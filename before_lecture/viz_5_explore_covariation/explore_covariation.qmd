...
alt.Chart(diamonds).mark_boxplot().encode(
    ...
    #The white lines in the boxplot is the median
    #of the distribution. Box plot only shows outliers
    #if the value is 1.5 more than the median
```
...
penguins_2 = penguins.groupby(['year', 'species'])['body_mass_g'].mean().reset_index()


alt.Chart(penguins_2).mark_bar().encode(
    x='year:O',
    y='body_mass_g:Q',
    color='species:N',
    tooltip=['year', 'species', 'body_mass_g']
).properties(
    title='Average Body Mass of Penguins by Species Over Years'
)
